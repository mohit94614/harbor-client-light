...
class HarborClient(object):
    ...
    def get_project_id_from_name(self, project_name):
        registry_data = requests.get(
            '%s://%s/api/v2.0/projects?project_name=%s' %
    ...
    def search(self, query_string):
        result = None
        path = '%s://%s/api/v2.0/search?q=%s' % (self.protocol, self.host,
                                            query_string)
    ...
    def get_projects(self, project_name=None, is_public=None):
        ...
        result = None
        path = '%s://%s/api/v2.0/projects' % (self.protocol, self.host)
        response = requests.get(path,
                                cookies={'beegosessionID': self.session_id})
        if response.status_code == 200:
            result = response.json()
    ...
    def check_project_exist(self, project_name):
        result = False
        path = '%s://%s/api/v2.0/projects?project_name=%s' % (
            self.protocol, self.host, project_name)
    ...
    def set_project_publicity(self, project_id, is_public):
        result = False
        path = '%s://%s/api/v2.0/projects/%s/publicity?project_id=%s' % (
    ...
    def get_users(self, user_name=None):
        # TODO: support parameter
        result = None
        path = '%s://%s/api/v2.0/users' % (self.protocol, self.host)
        response = requests.get(path,
                                cookies={'beegosessionID': self.session_id})
        if response.status_code == 200:
            result = response.json()
    ...
    def update_user_profile(self, user_id, email, realname, comment):
        ...
        result = False
        path = '%s://%s/api/v2.0/users/%s?user_id=%s' % (self.protocol, self.host,
                                                    user_id, user_id)
    ...
    def change_password(self, user_id, old_password, new_password):
        result = False
        path = '%s://%s/api/v2.0/users/%s/password?user_id=%s' % (
    ...
    def promote_as_admin(self, user_id):
        ...
        result = False
        path = '%s://%s/api/v2.0/users/%s/sysadmin?user_id=%s' % (
            self.protocol, self.host, user_id, user_id)
        response = requests.put(path,
                                cookies={'beegosessionID': self.session_id})
        if response.status_code == 200:
    ...
    def get_repositories(self, project_id, query_string=None):
        # TODO: support parameter
        result = None
        path = '%s://%s/api/v2.0/repositories?project_id=%s' % (
    ...
    def delete_repository(self, repo_name, tag=None):
        ...
        result = False
        path = '%s://%s/api/v2.0/repositories?repo_name=%s' % (self.protocol,
                                                          self.host, repo_name)
        response = requests.delete(path,
                                   cookies={'beegosessionID': self.session_id})
        if response.status_code == 200:
    ...
    def get_repository_tags(self, repo_name):
        result = None
        path = '%s://%s/api/v2.0/repositories/tags?repo_name=%s' % (
    ...
    def get_repository_manifests(self, repo_name, tag):
        result = None
        path = '%s://%s/api/v2.0/repositories/manifests?repo_name=%s&tag=%s' % (
    ...
    def get_top_accessed_repositories(self, count=None):
        result = None
        path = '%s://%s/api/v2.0/repositories/top' % (self.protocol, self.host)
        if count:
            path += "?count=%s" % (count)
        response = requests.get(path,
                                cookies={'beegosessionID': self.session_id})
    ...
    def get_logs(self, lines=None, start_time=None, end_time=None):
        result = None
        path = '%s://%s/api/v2.0/logs' % (self.protocol, self.host)
        response = requests.get(path,
                                cookies={'beegosessionID': self.session_id})
        if response.status_code == 200:
            result = response.json()
